        ...
        for k=-abs(n)-abs(2*m):0
            ...
            if k==n+2*m && m==0 && n~=0 && mod(n,2)==0
                if isequal(Answer,"p*L")
                    sprintf('The %d homology of (%d,%d) sphere is p^*L',k,n,m)
                    found=found+1;
                else
                    disp(Answer)
                    error('The %d homology of (%d,%d) sphere is not p^*L',k,n,m)
                end
            end
            
            if k==n+2*m && n<=-3 && m==0 && mod(n,2)==1
                if isequal(Answer,"p*L_-")
                    sprintf('The %d homology of (%d,%d) sphere is p^*L_-',k,n,m)
                    found=found+1;
                else
                    disp(Answer)
                    error('The %d homology of (%d,%d) sphere is not p^*L_-',k,n,m)
                end
            end

...
%[S0,Q0]=Smithoptimal(D0,0,1); 
%[S0,~,Q0]=Smithoptimalhedge(D0,0,1); 
[S0,~,Q0,~,Q0i]=Smithoptimalhedgeall(D0,0,1,0,1); 
%SmithVariables{1}=Q0;
SmithVariables{1}=Q0i;

kernelS0=zeros(M); %PA=SQ^{-1} means Ker(A)=Q*Ker(S).
...
Kernel=Q0*kernelS0; %i.e. Ker(A)=Q*Ker(S),  %The columns of Kernel are a basis for Ker(D0) plus some zero vectors. Let's call the nonzero ones x,y,z,... Our generators will be in terms of these. So Kernel=(x,y,z,...) with zeros too
%D1=round(Q0\D1); %Rounding is necessary when say multiplying u_lambda by u_lambda^{-15} to verify it's a generator
D1=Q0i*D1;
...
%[S1,P1]=Smithoptimal(D1,1,0); %P1i changes from a new basis we denote by u,v,w,... to x,y,z,...
%[S1,P1,~]=Smithoptimalhedge(D1,1,0); 
[S1,P1,~,P1i,~]=Smithoptimalhedgeall(D1,1,0,1,0); 
SmithVariables{3}=P1;

%Kernel=round(Kernel/P1);
Kernel=Kernel*P1i;
